$(document).ready(function() {
    ...
    $('#month_spend').change(function() {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function(data) {
                var str = '<select name="month_spend" id="month_spend"><option value="Tất cả" selected>Tất cả</option>'
                for (i = 0; i < data.length; i++) {
                    var spend = data[i]
                    str = str + '<option value="' + spend['ngaymua'].substr(0, 7) + '">' + spend['ngaymua'].substr(0, 7) + '</option>'
                }

                str = str + '</select><table id="content_table_spend"><tr> <th id="STT">STT</th> <th id="Loai">Loại chi tiêu</th><th id="content">Nội dung</th><th id="ngay">Ngày mua</th><th id="tong">Tổng tiền</th></tr>'
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    if (ngay == text) {
                        if (idND == spend['idND']) {
                            stt++
                            str = str + "<tr><td>" + stt + "</td><td>" + spend['Loai'] + "</td><td>" + spend['noidung'] + "</td><td>" + spend['ngaymua'] + "</td><td>" + spend['tongtien'] + " VNĐ</td></tr>"
